...
void PriorityQueue<T>::remove(T element) {
	for (typename vector<T>::iterator it = m_heap.begin(); it != m_heap.end(); ++it) {
		if (*it == element) {
			m_heap.erase(it);
			break;
		}
	...
	//assert(success == 1);
	this->percolate_up((int)m_heap.size());
	this->percolate_down(0);
  //
...
void PriorityQueue<T>::percolate_up(int i) {
	while (i > 0) {
		int parent = i / 2 - 1;
		int swp = m_heap.size();
		if (parent - 1 > 0 && m_heap[parent] < m_heap[i]) {
			swp = i;
		} else {
			swp = parent;
		}
		if (m_heap[swp] > m_heap[i]) {
			swap(m_heap[swp], m_heap[i]);
			i = swp;
		} else {
			break;
		}
...
void PriorityQueue<T>::percolate_down(int i) {
	while (i * 2 + 1 < int(m_heap.size())) {
		int child = i * 2 + 1;
		int swp = -1;
		if (child + 1 < int(m_heap.size()) && m_heap[child+1] < m_heap[child]) {
			swp = child + 1;
		} else {
			swp = child;
		}
