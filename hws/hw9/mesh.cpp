...
bool Mesh::Collapse() {
  ...
  int numTri = numTriangles();

  Vertex *a = e->getV1();
  Vertex *b = e->getV2();

  vector<vector<Vertex*> > vertices;
  vector<vector<Vertex*> >::iterator itr_v;

  // Find the average of the two points a and b
  ...
  for (triangles_set::iterator itr = copy_tri.begin(); itr != copy_tri.end(); itr++) {
	  ...
	  if (t->HasVertex(a) && t->HasVertex(b)) {
		  RemoveTriangle(t);
		  continue;
	  } else if (t->HasVertex(a) || t->HasVertex(b)) {
		  // If the triangle touches only one of the points, store the triangle's 
		  // vertices in a vector
		  vector<Vertex*> vex;
		  for (int i = 0; i < 3; ++i) {
			  vex.push_back(t->getVertex(i));
		  }
		  // The vector of the triangle's vertices is stored in another vector which 
		  // essentially groups vertices by triangles
		  vertices.push_back(vex);
		  // Remove all of the triangles touching a or b
		  RemoveTriangle(t);
	  }
  }
  // Remove the edge
  ...
  ReCalculateEdges(b_edges);
  CollectEdgesWithVertex(a,a_edges);
  ReCalculateEdges(a_edges);

  // Remove all of the edges associated with points a or b
  ...
  // Go through all of the stored triangle vertices and replace the vertices
  // that used to be a or b with the new a point
  for (itr_v = vertices.begin(); itr_v != vertices.end(); ++itr_v) {
	  vector<Vertex*> vex = *itr_v;
	  // Vector stores the two points that are not a or b
	  vector<Vertex*> tri;
	  for (int i = 0; i < 3; ++i) {
		  if (vex[i] != a && vex[i] != b) {
			  // If this vertex is not a or b, push onto this vector
			  tri.push_back(vex[i]);
		  }
	  }
	  // Add the triangle to the mesh using the two points found and 
	  // point a as the third vertex
	  AddTriangle(tri[0],tri[1],a);
  }
...
Edge* Mesh::FindEdge() const {
  ...
  // these two different methods are handled in the edge
    ...
    // perform a priority queue to find the best edge
		...
		//for (edges_map::const_iterator it = edges.begin(); it != edges.end(); ++it) {
		//	edges_pq.push((*it).second);
		//}
		Edge *e = edges_pq.top();
		if (e->isLegal() && (answer == NULL || e->getLength() < answer->getLength())) {
			answer = e;
