...
bool find_paths(const vector<vector<bool> > &grid, vector<Point> &points, int x, int y, int &paths) {
	bool works = false;
	Point newPoint(x,y);
	if ( x == 0 && y == 0) {
		++paths;
	...
	if (y == 0) {
		works = find_paths(grid,points,x-1,y,paths);
	} else if ( x == 0 ) {
		works = find_paths(grid,points,x,y-1,paths);
	} else {
		works = find_paths(grid,points,x-1,y,paths);
		if (!works) { 
			works = find_paths(grid,points,x,y-1,paths);
...
bool find_num_paths(const vector<vector<bool> > &grid, int x, int y, int &paths) {
	...
	if (y == 0) {
		find_num_paths(grid,x-1,y,paths);
	} else if ( x == 0 ) {
		find_num_paths(grid,x,y-1,paths);
	} else {
		find_num_paths(grid,x-1,y,paths); 
		find_num_paths(grid,x,y-1,paths);
	}
...
int main(int argc, char* argv[]) {
  ...
  int paths = 0;
  vector<Point> points;
  find_num_paths(blocked_grid,start_x,start_y,paths);
  cout << endl << "Number of paths: " << paths << endl;
  find_paths(blocked_grid,points,start_x,start_y,paths);
  cout << endl << "Path: " << endl;
