...
public:
  ...
  list_iterator(Node<T>* p, dslist<T>* ds) : ptr_(p), dslist_(ds) {}
  ...
  list_iterator<T>& operator--() { // pre-decrement, e.g., --iter
	  if (ptr_ == NULL) { ptr_ = dslist_->tail_; return *this; }
    ptr_ = ptr_->prev_;
    return *this;
  }
  list_iterator<T> operator--(int) { // post-decrement, e.g., iter--
    list_iterator<T> temp(*this);
	if (ptr_ == NULL) { ptr_ = dslist_->tail_; return temp; }
    ptr_ = ptr_->prev_;
    return temp;
  }
