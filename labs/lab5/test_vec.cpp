...
template <class T> 
int remove_matching_elements(Vec<T> &A,T x) 
{
	typename Vec<T>::iterator it = A.begin();
	size_type counter = 0;
...
int main() {
  ...
  //v4.push_back(2);
  cout << endl << "Contents of vector v4: ";
  for(Vec<int>::size_type j = 0; j < v4.size(); ++j) {
	  cout << " " << v4[j];
  }
  cout << endl;
  v4.print();
  remove_matching_elements(v4,2);
  cout << endl << "Removing 2: ";
  for (Vec<int>::size_type k = 0; k < v4.size(); ++k) {
	  cout << " " << v4[k];
  }
  cout << endl;
  v4.print();
